# ==========================================================
# train_ablation.py
# RLCarla — ABLATION training driver for the DQL-E Section D
#           ablation study (derived from the v15 signal-
#           compliance training script).
#
# WHAT THIS FILE IS
#   A copy of the v15 DQL-E training script with three small,
#   clearly-marked additions that let a single script run the
#   ablation variants without duplicating the whole recipe.
#   The underlying training recipe is UNCHANGED from v15.
#
# ABLATION MODES (CLI: --ablation)
#   none        : unmodified reference recipe (sanity / baseline)
#   no_eta      : remove the eta WARMUP RAMP — eta jumps straight
#                 to ETA_END once critic pre-training ends, instead
#                 of ramping 0 -> ETA_END over ETA_WARMUP steps.
#                 (During pre-training eta stays 0 in both cases.)
#   no_pretrain : PRE_TRAIN_CRITIC_STEPS = 0 — the actor trains
#                 against the critic from the first full update,
#                 with no critic warm-up phase.
#
# ISOLATION
#   Each run writes to checkpoints_ablate_<tag> and
#   runs/ablate_<tag> (and its own replay buffer), so the
#   v14 / v15b production checkpoints are NEVER touched.
#
# USAGE
#   python3 train_ablation.py --ablation no_eta      --episodes 700 --tag no_eta
#   python3 train_ablation.py --ablation no_pretrain --episodes 700 --tag no_pretrain
#
...
class TrainConfig:
    """
    Diffusion-QL v15 — Signal-Compliance (lane-based lights).
    ...
    LOG_DIR        = "runs/rlcarla_v15"
    # Replay-buffer persistence file
    BUFFER_PATH    = "checkpoints_v15/replay_buffer.npz"
    DEVICE = torch.device(
...
# ==========================================================
# ABLATION OVERRIDES  (Section D ablation study)
# ----------------------------------------------------------
#   --ablation no_eta      : hold eta fixed at ETA_END (full
#                            guidance immediately, no warmup ramp)
#   --ablation no_pretrain : PRE_TRAIN_CRITIC_STEPS = 0 (actor
#                            trains against the critic from step 0)
#   --ablation none        : unmodified reference recipe
#
# Each run writes to its own checkpoints_ablate_<tag> /
# runs/ablate_<tag> so v14 / v15b checkpoints are never touched.
# ==========================================================
_ap = argparse.ArgumentParser()
_ap.add_argument("--ablation", choices=["none", "no_eta", "no_pretrain"],
                 default="none")
_ap.add_argument("--episodes", type=int, default=cfg.MAX_EPISODES)
_ap.add_argument("--tag", type=str, default="ref")
_args, _ = _ap.parse_known_args()

ABLATION = _args.ablation
cfg.MAX_EPISODES   = _args.episodes
cfg.CHECKPOINT_DIR = f"checkpoints_ablate_{_args.tag}"
cfg.LOG_DIR        = f"runs/ablate_{_args.tag}"
cfg.BUFFER_PATH    = f"checkpoints_ablate_{_args.tag}/replay_buffer.npz"

if ABLATION == "no_pretrain":
    cfg.PRE_TRAIN_CRITIC_STEPS = 0

logger.info(
    f"[ABLATION] mode={ABLATION} tag={_args.tag} "
    f"episodes={cfg.MAX_EPISODES} "
    f"pretrain_steps={cfg.PRE_TRAIN_CRITIC_STEPS} "
    f"ckpt={cfg.CHECKPOINT_DIR}"
)
...
def get_eta(total_steps):
    ...
    Ablation 'no_eta' removes the warmup RAMP: eta jumps to its
    full value (ETA_END) as soon as critic pre-training ends,
    instead of ramping 0 -> ETA_END over ETA_WARMUP steps. During
    pre-training eta stays 0 in both cases (no actor updates yet).
    """
    if total_steps < cfg.PRE_TRAIN_CRITIC_STEPS:
        return 0.0
    if ABLATION == "no_eta":
...
def draw_hud(screen, font, info, total_steps, episode,
    ...
    lines = [
        f"[DQL-E v15] Ep : {episode} (+{episode-start_episode})",
        f"Phase       : {phase}",
...
pygame.display.set_caption(
    "RLCarla — Ablation DQL (Section D)"
)
...
logger.info(f"Algorithm        : DQL ABLATION ({ABLATION})")
...
logger.info(
    ...
    f"{' (RAMP DISABLED: held at ETA_END)' if ABLATION=='no_eta' else ''}"
)
logger.info(
    f"Critic reset     : every {cfg.CRITIC_RESET_FREQ}"
)
logger.info(f"Lights ON        : episode 0 (cycling R8/G10/Y2)")
...
try:
    ...
    env.close()
    pygame.quit()
    logger.info("DQL ablation training finished.")
