...
# ==========================================================
# run_eval_ppo.py
# PPO Evaluation Runner
#
# Identical evaluation logic to DQL-E and SAC.
# Only difference: loads PPO agent.
#
# Usage:
#   python3 run_eval_ppo.py
#   python3 run_eval_ppo.py --scenario heavy
#   python3 run_eval_ppo.py --noise
#   python3 run_eval_ppo.py --slow
#
...
logger = logging.getLogger("eval_ppo")
...
def load_agent():
    ...
    policy_path = os.path.join(
        CHECKPOINT_DIR, "ppo_policy_9999.pth"
    )
    if not os.path.exists(policy_path):
        raise FileNotFoundError(
            f"PPO checkpoint not found: {policy_path}"
        )
    agent.load_model(CHECKPOINT_DIR, id=9999)
    agent.actor.eval()
    logger.info(
        "PPO model loaded (ppo_policy_9999.pth)"
    )
    return agent
...
def get_action(agent, obs):
    ...
    with torch.no_grad():
        ...
        action, _, _ = agent.actor(state_t)
...
def main(args):
    ...
    pygame.display.set_caption("PPO Evaluation")
